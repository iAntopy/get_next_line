...
/*   Updated: 2022/05/03 18:55:31 by iamongeo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//		printf("joined line : %s\n", *line);
		*cur_len += idx;
		if (nl_found)
		{
//			printf("nl found at idx %zu, n_chrs : %zu, cur_len %zd\n", idx, n_chrs, *cur_len);
//			printf("rems[fd] ptr before : %p\n", rems[fd]);
			if (!ft_strndup(&rems[RD_IDX][idx], n_chrs - idx, &rems[fd]))
			{
//				printf("ERROR : strdup failed!");
				return (E_MALLOC_ERR);
			}
//			printf("rems[fd] ptr after : %p\n", rems[fd]);
//			printf("rem : %s\n", rems[fd]);
			return (SG_RETURN);
		}
//		else
//		{
//			printf("no nl found reading fd. n_chrs : %zu\n", n_chrs);
//			rems[fd] = NULL;
//		}
	}
}
...
//	if (status == SG_EOF && line)
//		return (line);
	if (line && status != E_MALLOC_ERR)
		return (line);
	return (NULL);
/*
	if (status == E_MALLOC_ERR || status == SG_EOF || status == E_INVALID_FD)
	{
		if (status == SG_EOF)
			printf("fd %d reached EOF\n", fd);
		else if (status == E_MALLOC_ERR)
			printf("scan_rem failed with malloc error.\n");
		else if (status == E_INVALID_FD)
			printf("INVALID FD !\n");
		return (NULL);
	}
	printf("EXIT fd %d\n", fd);
